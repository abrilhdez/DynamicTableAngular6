...
export class TableDataSource extends DataSource<TableElement> {


  private rowsSubject: BehaviorSubject<TableElement[]>;
  datasourceSubject: Subject<any[]>;
  columnsConfig: any[];
  tableConfig: any;
  displayedColumns: string[];
  private dataConstructor: new () => any;
  ...
  constructor(
    ...
    if (data && data.length > 0){
      this.displayedColumns= columnsConfig.filter(f=>f.is_hidden ===false);
   ...
   */
  confirmCreate(row: TableElement): boolean {
   ...
   */
  confirmEdit(row: TableElement): boolean {
 ...
 */
   ...
   */
  updateDatasource(data: any[], options = { emitEvent: true }): void {
   ...
   * @param source
   */
  private existsNewElement(source: TableElement[]): boolean {
   ...
   * @param source
   */
  private getIndexFromRowId(id: number, source: TableElement[]): number {
    if (id == -1) {
   ...
   */
  private updateRowIds(initialIndex: number, source: TableElement[]): void {
   ...
   */
  private getDataFromRows(rows: TableElement[]): any[] {
    return rows
      .filter(row => row.id != -1)
      .map<any>((row) => {
   ...
   */
  private updateDatasourceFromRows(rows: TableElement[]): void {
   ...
   */
  private getRowsFromData(arrayData: any[]): TableElement[] {
    return arrayData.map<TableElement>((data, index) => {
